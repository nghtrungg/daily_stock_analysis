...
function formatNotificationStatus(notification: AlertNotificationItem): string {
  if (notification.success) return 'Succeeded';
  if (notification.errorCode === 'cooldown_active') return 'Suppressed by cooldown';
  ...
  return (
    <AppPage className="space-y-5">
      <PageHeader
        ...
        title="Alert Center"
        description="Manage event, daily-indicator, watchlist, and portfolio alerts; run one-time tests; and review triggers recorded by background evaluations."
      />
      ...
      <Card title="Notification attempts" subtitle="Delivery results" variant="bordered" padding="md">
        {notificationsLoading ? <Loading label="Loading notification attempts" /> : null}
        {!notificationsLoading && notifications.length === 0 ? (
          <EmptyState
            icon={<BellRing className="h-6 w-6" />}
            title="No notification attempts"
            description="There are no notification-attempt details to show. Triggered alerts will still use the configured notification channels."
          />
        ) : null}
        {!notificationsLoading && notifications.length > 0 ? (
