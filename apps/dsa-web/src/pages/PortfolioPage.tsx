...
const ENABLE_LEGACY_BROKER_IMPORT = false;
const FALLBACK_BROKERS: PortfolioImportBrokerItem[] = [
  { broker: 'huatai', aliases: [], displayName: 'Huatai (legacy importer)' },
  { broker: 'citic', aliases: ['zhongxin'], displayName: 'CITIC (legacy importer)' },
  { broker: 'cmb', aliases: ['cmbchina', 'zhaoshang'], displayName: 'CMB (legacy importer)' },
];
...
const PORTFOLIO_LIMITATION_LABELS: Record<string, Record<PortfolioPageLanguage, string>> = {
  ...
  fx_and_cost_basis_partial: {
    zh: 'FX and cost basis are partial',
    en: 'FX and cost basis are partial',
  },
  sector_and_risk_metrics_limited: {
    zh: 'Sector and risk metrics are limited',
...
const DECISION_SIGNAL_MARKETS = new Set<DecisionSignalMarket>(['vn', 'cn', 'hk', 'us', 'jp', 'kr', 'tw']);
type PortfolioAccountMarket = 'vn' | 'cn' | 'hk' | 'us' | 'jp' | 'kr' | 'tw';
...
const PortfolioPage: React.FC = () => {
  ...
  const [accountForm, setAccountForm] = useState({
    name: '',
    broker: 'Demo',
    market: 'vn' as PortfolioAccountMarket,
    baseCurrency: 'VND',
  });
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('The broker list was empty; using the built-in legacy CSV importers.');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('The broker list is unavailable; using the built-in legacy CSV importers.');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || 'Risk data is unavailable; showing the portfolio snapshot only.');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(`Analysis submitted for ${row.symbol}: ${task.taskId}`);
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('Select a specific account before adding or importing records.');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning('Select a specific account before deleting a record.');
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning('Select a specific account before deleting it.');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAccountCreateSuccess('Account created and selected.');
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || 'Could not create the account. Try again.');
      setAccountCreateSuccess(null);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
        ...
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            ...
            {hasAccounts ? (
              ...
              <span className="text-xs text-secondary">The new account will be selected automatically</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            <input
              ...
              placeholder="Broker (optional, for example Demo)"
              value={accountForm.broker}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, broker: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder="Base currency (VND)"
            ...
            >
              <option value="vn">Vietnam market (vn)</option>
            </select>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <Card padding="md">
          ...
          <div className="text-xs text-secondary space-y-1">
            ...
            <div>{text.currency}: {snapshot?.currency || 'VND'}</div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="Stock code (for example FPT.VN)" value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="Trade price in VND (required)" value={tradeForm.price}
            ...
            <p className="text-xs text-secondary">Blank fees and taxes are treated as zero.</p>
        ...
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`Currency (default: ${writableAccount?.baseCurrency || 'account base currency'})`} value={cashForm.currency}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        {ENABLE_LEGACY_BROKER_IMPORT ? (
          <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Legacy broker CSV import</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  ...
                  <option value="huatai">Huatai (legacy importer)</option>
                )}
              </select>
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">Dry run (do not write)</label>
            </div>
            <div className="flex gap-2">
              ...
              <button type="button" className="btn-secondary flex-1"
                ...
                {csvCommitting ? 'Importing...' : 'Import records'}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                ...
                message={`${csvParseResult.recordCount} valid, ${csvParseResult.skippedCount} skipped, ${csvParseResult.errorCount} errors.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? 'CSV dry-run result' : 'CSV import result'}
                message={`${csvDryRun ? 'Validation' : 'Write'}: ${csvCommitResult.insertedCount} inserted, ${csvCommitResult.duplicateCount} duplicates, ${csvCommitResult.failedCount} failed.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
          ...
          <Card padding="md">
            <h3 className="text-sm font-semibold text-foreground mb-3">Vietnam broker CSV import</h3>
            <p className="text-sm text-secondary-text">
              Disabled until a Vietnamese broker adapter is implemented and validated. Use manual trade, cash, and corporate-action entry in the meantime.
            </p>
          </Card>
        )}
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? 'Loading...' : 'Refresh records'}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-[11px] text-secondary">
              {writeBlocked ? 'Deletion is available only in a single-account view.' : 'Delete an incorrect record, then enter it again.'}
            </div>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `Delete the ${formatSideLabel(item.side)} record for ${item.symbol} on ${item.tradeDate} (quantity ${item.quantity}, price ${item.price})?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `Delete the cash movement on ${item.eventDate} (${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency})?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `Delete the ${formatCorporateActionLabel(item.actionType)} for ${item.symbol} on ${item.effectiveDate}?`,
                      })}
                    >
                      Delete
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title="No records"
                      description="Adjust the filters or add a trade, cash movement, or corporate action."
      ...
      <ConfirmDialog
        ...
        title="Delete incorrect record"
        message={pendingDelete?.message || 'Delete this record?'}
