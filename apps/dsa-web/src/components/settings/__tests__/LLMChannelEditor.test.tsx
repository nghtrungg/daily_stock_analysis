...
import { fireEvent, render as renderWithTestingLibrary, screen, waitFor, within } from '@testing-library/react';
...
function render(ui: ReactElement) {
  return renderWithTestingLibrary(ui, {
    wrapper: ({ children }: { children: ReactNode }) => (
      <UiLanguageProvider lockedLanguage="en">{children}</UiLanguageProvider>
    ),
  });
}
...
describe('LLMChannelEditor', () => {
  ...
  it('shows help dialogs for channel editor fields', async () => {
    ...
    expect(screen.getByText('Endpoint root for this channel.')).toBeInTheDocument();
    ...
    expect(screen.getByText('Unified runtime sampling temperature.')).toBeInTheDocument();

    fireEvent.keyDown(document, { key: 'Escape' });
    fireEvent.click(await screen.findByRole('button', { name: 'View Runtime capability checks configuration help' }));

    expect(screen.getByRole('dialog', { name: 'Runtime Capability Checks' })).toBeInTheDocument();
    expect(screen.getByText('Select capabilities and run the check. It sends real LLM requests.')).toBeInTheDocument();
  });
  ...
  it('hides LiteLLM wording when advanced YAML routing is enabled', () => {
    ...
    expect(screen.getByText(/An advanced model-routing YAML configuration is active/i)).toBeInTheDocument();
    expect(screen.getByText(/Runtime primary, fallback, vision, and temperature settings still come from the general fields below/i)).toBeInTheDocument();
  ...
  it('rejects bare mixed Hermes route before saving runtime generation config', async () => {
    ...
    expect(await screen.findByText(/Mixed Hermes and non-Hermes routes are not supported as primary or fallback models/i)).toBeInTheDocument();
  ...
  it('does not test runtime-only masked Hermes secrets from the settings UI', async () => {
    ...
    const messages = await screen.findAllByText(/A runtime-injected Hermes key cannot be returned/i);
  ...
  it('shows provider capability badges, official sources, and config hints', async () => {
    ...
    expect(await screen.findByText('Configuration reference')).toBeInTheDocument();
    expect(screen.getByText('OpenAI Compatible')).toBeInTheDocument();
    expect(screen.getByText('Aggregator')).toBeInTheDocument();
    expect(screen.getByText('Model discovery')).toBeInTheDocument();
    expect(screen.getByText(/The model list and model visibility depend on account permissions and the API key/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'OpenRouter Models API' })).toHaveAttribute(
      'href',
      'https://openrouter.ai/docs/api/api-reference/models/get-models',
    );
    expect(screen.getByText(/Capability labels are configuration guidance only; they do not confirm runtime support/i)).toBeInTheDocument();
  });
  ...
  it('does not show provider metadata for custom or unknown channels', async () => {
    ...
    expect(screen.queryByText(/Capability labels are configuration guidance only/i)).not.toBeInTheDocument();
  });
  ...
  it('clears active Hermes unsupported multi-key and extra-header env keys on save', async () => {
    update.mockResolvedValue({
      ...
      warnings: [
        'Cleaned unsupported Hermes Phase 3 settings: LLM_HERMES_API_KEYS, LLM_HERMES_EXTRA_HEADERS. The Hermes reserved channel supports one LLM_HERMES_API_KEY and does not support multiple keys or extra headers. Restore old values manually from an .env backup, Git history, or a desktop export, but non-empty LLM_HERMES_API_KEYS or LLM_HERMES_EXTRA_HEADERS will still be rejected by backend validation.',
      ],
    });
    ...
    expect(await screen.findByText(/Cleaned unsupported Hermes Phase 3 settings/i)).toBeInTheDocument();
    expect(screen.getByText(/Restore old values manually from an \.env backup/i)).toBeInTheDocument();
  });
  ...
  it('prompts when bare runtime models loosely match canonical OpenAI route aliases', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /primaryopenai 1 model configured/i }));
    fireEvent.change(screen.getByLabelText('Base URL'), {
    ...
    await waitFor(() => {
      expect(screen.getByText('The current runtime model uses a non-canonical route alias. Select a canonical model from the list.')).toBeInTheDocument();
    });
  ...
  it('shows cleanup warning and restore path after stale runtime models are removed on save', async () => {
    update.mockResolvedValue({
      ...
      updatedKeys: ['LLM_DEEPSEEK_MODELS', 'LITELLM_MODEL'],
      warnings: [
        'Removed stale runtime model references from the primary, Agent primary, Vision, and fallback models. To restore them, add the corresponding channel models and select them again, or restore the previous LLM settings from a desktop export or .env backup.',
      ],
    });
    ...
    expect(screen.getByText(/Removed stale runtime model references/i)).toBeInTheDocument();
    expect(screen.getByText(/desktop export or \.env backup/i)).toBeInTheDocument();
  });

  it('keeps save warnings visible after onSaved-driven refresh', async () => {
    const warningMessage = 'Removed stale runtime model references from the primary, Agent primary, Vision, and fallback models.';
    const initialItems = [
      { key: 'LLM_CHANNELS', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_PROTOCOL', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_BASE_URL', value: 'https://api.deepseek.com' },
  ...
  it('shows structured troubleshooting hint when channel auth fails', async () => {
    ...
    expect(await screen.findByText(/Chat completion · Authentication failed.*LLM authentication failed/i)).toBeInTheDocument();
    expect(screen.getByText(/Check that the API key is correct/i)).toBeInTheDocument();
    expect(screen.queryByText(/reorder the models or remove it/i)).not.toBeInTheDocument();
  });
  ...
  it('shows tested model and model-availability hints when a model is disabled', async () => {
    ...
    expect(await screen.findByText(/Chat completion · Model unavailable.*LLM channel test failed/i)).toBeInTheDocument();
    expect(screen.getByText(/Model tested: openai\/deepseek-ai\/DeepSeek-V3/i)).toBeInTheDocument();
    expect(screen.getByText(/The basic connection test uses the first model in the list: deepseek-ai\/DeepSeek-V3/i)).toBeInTheDocument();
    expect(screen.getByText(/The basic connection test checks only the first model in the list/i)).toBeInTheDocument();
    expect(screen.getByText(/reorder the models or remove it/i)).toBeInTheDocument();
    expect(screen.getByText(/This account cannot access the model/i)).toBeInTheDocument();
    expect(screen.queryByText(/Base URL, proxy, TLS certificate/i)).not.toBeInTheDocument();
  ...
  it('shows provider blocked troubleshooting without network or model-list hints', async () => {
    ...
    expect(await screen.findByText(/Chat completion · Request blocked/i)).toBeInTheDocument();
    expect(screen.getByText(/Model tested: openai\/gpt-5\.5/i)).toBeInTheDocument();
  ...
  it('shows focused quota exceeded troubleshooting hints', async () => {
    ...
    expect(await screen.findByText(/provider reports that the quota is exhausted/i)).toBeInTheDocument();
    expect(screen.queryByText(/reorder the models or remove it/i)).not.toBeInTheDocument();
  });
  ...
  it('does not request runtime capabilities during the basic connection test', async () => {
    ...
    await screen.findByText(/Connection successful · openai\/gpt-4o-mini/i);
  ...
  it('runs explicit runtime capability checks and shows detailed hints', async () => {
    ...
    expect(await screen.findByText(/Capability checks complete: 1 passed \/ 1 failed \/ 0 skipped/i)).toBeInTheDocument();
  ...
  it('shows skipped runtime capabilities when the base test fails', async () => {
    ...
    expect(await screen.findByText(/Capability checks complete: 0 passed \/ 0 failed \/ 1 skipped/i)).toBeInTheDocument();
    expect(screen.getByText('JSON Skipped')).toBeInTheDocument();
    expect(screen.getByText(/provider rejected this API key/i)).toBeInTheDocument();
  ...
  it('keeps manual model input available when discovery fails', async () => {
    ...
    await screen.findByText(/Model discovery · Unsupported protocol.*Model discovery is not supported for this protocol/i);
    expect(screen.getByText(/Automatic model discovery currently supports only OpenAI-compatible and DeepSeek channels/i)).toBeInTheDocument();
  ...
  it('maps discovery format errors to the /models troubleshooting hint', async () => {
    ...
    expect(await screen.findByText(/Response parsing · Invalid response format.*Failed to parse \/models response/i)).toBeInTheDocument();
    expect(screen.getByText(/This channel returned an incompatible \/models response/i)).toBeInTheDocument();
  });
  ...
  it('maps discovery empty responses to the /models troubleshooting hint', async () => {
    ...
    expect(screen.getByText(/This channel's \/models endpoint returned no usable model IDs/i)).toBeInTheDocument();
    expect(screen.queryByText(/additional response mode/i)).not.toBeInTheDocument();
  });
