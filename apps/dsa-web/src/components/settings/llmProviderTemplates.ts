...
export const LLM_PROVIDER_CAPABILITY_LABELS: Record<LLMProviderCapability, { label: string; hint: string }> = {
  'openai-compatible': {
    ...
    hint: 'Configure the Base URL for the OpenAI-compatible endpoint without appending /chat/completions.',
  },
  aggregator: {
    label: 'Aggregator',
    hint: 'Model availability, routing, and pricing may vary by account permissions and platform policy.',
  },
  'official-api': {
    label: 'Official API',
  ...
  'model-discovery': {
    ...
    hint: 'Can request a model list from /models; actual results still depend on account permissions and the API key.',
  },
  vision: {
    label: 'Vision guidance',
    hint: 'This provider is commonly used for vision workloads; exact capabilities still depend on the account and model.',
  },
  'local-runtime': {
    label: 'Local runtime',
    hint: 'The current runtime must be able to reach the corresponding local service.',
  },
};
...
export const LLM_PROVIDER_TEMPLATES: LLMProviderTemplate[] = [
  ...
  },
  {
    ...
    configHint:
      'The same ANSPIRE_API_KEYS value can be reused for search and LLM channels. These models and gateways are examples; availability depends on your account and provider console. Test the connection before saving.',
    officialSources: [
  ...
  },
  {
    ...
    capabilities: ['openai-compatible', 'model-discovery'],
    configHint: 'The model list and model visibility depend on account permissions and the API key.',
  ...
  },
  {
    ...
    configHint: 'Your machine, Docker container, or self-hosted runner must be able to reach the Ollama service.',
