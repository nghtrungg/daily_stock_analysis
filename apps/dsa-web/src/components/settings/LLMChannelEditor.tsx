...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: 'Checks whether response_format JSON output is supported.' },
  { value: 'tools', label: 'Tools', hint: 'Checks whether function and tool calling are supported.' },
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = 'A runtime-injected Hermes key cannot be returned. Re-enter it here or save it to .env before testing from Settings.';
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-[11px] text-secondary-text">
            {modelCount > 0 ? `${modelCount} model${modelCount === 1 ? '' : 's'} configured` : 'No models configured'}
          </p>
        </div>
        ...
        <span className="flex shrink-0 items-center gap-2">
          ...
          {!hasKey && channel.protocol !== 'ollama' ? <Badge variant="warning">Key missing</Badge> : null}
          {testState?.status !== 'idle' ? (
            <Badge variant={statusVariant}>
              {testState?.status === 'success' ? 'Connected' : testState?.status === 'error' ? 'Connection failed' : 'Testing'}
            </Badge>
          ) : null}
        </span>
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                Capability labels are configuration guidance only; they do not confirm runtime support.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? 'Optional for local Ollama' : 'Separate multiple keys with commas'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.status === 'loading' ? 'Loading...' : 'Discover models'}
              </Button>
              ...
              >
                {discoveryState?.text || 'OpenAI-compatible channels that support `/models` can discover models automatically.'}
              </span>
            </div>
            {discoveryState?.hint ? (
            ...
            {discoveredModels.length > 0 ? (
              <div>
                <HelpLabel
                  label="Available models (select multiple)"
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? 'Add custom model names that are not listed above. They will still be saved as a comma-separated list.'
                  : 'Enter the model list manually if this channel does not support discovery or the request fails.'
              }
            />
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    The basic connection test uses the first model in the list: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  Real LLM requests are sent only when you run this check. Multiple checks may take 20-40 seconds.
                </p>
              </div>
              <Button
                type="button"
...
const LLM_STAGE_LABELS: Record<string, string> = {
  ...
  capability_stream: 'Streaming capability',
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'Check that the API key is correct, contains no extra spaces, and has any required organization or project permissions.',
  timeout: 'Retry the request. If timeouts continue, check the Base URL, network proxy, provider region, and local firewall.',
  quota: 'Check the account balance, plan quota, RPM/TPM limits, and concurrency settings, then retry later if needed.',
  model_not_found: 'Confirm that the model name matches the channel protocol, then use Discover models to verify the available models.',
  empty_response: 'The channel connected but returned no content. Try a compatible model or disable any additional response mode before testing again.',
  network_error: 'Check the Base URL, proxy, TLS certificate, relay gateway, and local network policy, then retry.',
  invalid_config: 'Complete the protocol, Base URL, API key, and model settings before running the test.',
  unsupported_protocol: 'Automatic model discovery currently supports only OpenAI-compatible and DeepSeek channels. Maintain the model list manually for this protocol.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'The API key is empty or the comma-separated list contains no usable keys. Enter at least one valid key before testing.',
  api_key_rejected: 'The provider rejected this API key. Check the key, organization or project permissions, region, and account status.',
  rate_limit: 'The provider applied an RPM, TPM, or concurrency limit. Reduce the request rate or retry later.',
  insufficient_balance: 'The provider reports insufficient balance, billing credit, or quota. Check the account balance and plan status.',
  quota_exceeded: 'The provider reports that the quota is exhausted. Check the account plan, remaining allowance, and project quota.',
  provider_blocked: 'The provider or relay gateway blocked the request. Check account risk controls, regional restrictions, model permissions, gateway policy, content safety policy, and request-source restrictions.',
  dns_error: 'DNS resolution failed. Check the Base URL domain, network proxy, and DNS configuration.',
  tls_error: 'The TLS or certificate handshake failed. Check the HTTPS certificate, relay gateway, and corporate proxy policy.',
  connection_refused: 'The target service refused the connection. Check the Base URL port, service process, and firewall.',
  model_access_denied: 'This account cannot access the model. Confirm that the model is enabled, visible to the account, and not disabled.',
  ...
  capability_unsupported: 'The current model or compatibility layer does not support this capability. Basic text connections are unaffected; use another model or disable the capability dependency.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? 'This channel returned an incompatible /models response. Enter the model list manually.'
      : 'The response structure does not match the expected format. Confirm that this channel supports the Chat Completions API.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return 'This channel\'s /models endpoint returned no usable model IDs. Check that the Base URL points to a compatible model-list endpoint, or enter the model list manually.';
  }
...
function buildLlmTestHint(result: {
  ...
  const modelHint = testedModel ? `Model tested: ${testedModel}.` : '';
  const scopeInfo = 'The basic connection test checks only the first model in the list.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? 'If this model is unavailable, reorder the models or remove it before retrying.'
    : '';
...
function buildLlmFailureText(result: {
  ...
  if (result.error && result.error !== result.message) {
    return `${prefix}: ${summary} (original summary: ${result.error})`;
  }
...
function summarizeCapabilityResults(results: Partial<Record<LLMCapabilityCheck, LLMCapabilityCheckResult>>): string {
  ...
  return `Capability checks complete: ${passed} passed / ${failed} failed / ${skipped} skipped`;
}
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel names are required and may contain only letters, numbers, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes and non-Hermes routes are not supported as primary or fallback models. Select either a Hermes-only or non-Hermes-only route.' });
        return;
      }
      ...
      if (nonCanonicalRouteAlias) {
        setSaveMessage({ type: 'local-error', text: 'The current runtime model uses a non-canonical route alias. Select a canonical model from the list.' });
        return;
      }
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'The current primary model is not available from an enabled channel. Select another model.' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'One or more fallback models are invalid. Select the fallback models again.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'The current vision model cannot include a Hermes deployment. Select a non-Hermes-only route.' });
        return;
      }
    }
    ...
    try {
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: result.success ? 'success' : 'error',
          text: result.success
            ? `Discovered ${result.models.length} model${result.models.length === 1 ? '' : 's'}${result.latencyMs ? ` · ${result.latencyMs} ms` : ''}`
            : buildLlmFailureText(result),
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: 'error',
          text: parsed.message || 'Model discovery failed',
          hint: undefined,
          models: previous[channel.id]?.models || [],
        },
      }));
      ...
      setCapabilityStates((previous) => ({
        ...
        [channel.id]: {
          ...
          text: Object.keys(capabilityResults).length > 0
            ...
            : result.success
              ? 'No capability-check result was returned'
              : buildLlmFailureText(result),
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            Add provider channels to discover and select models automatically, or enter models manually. Changes are synchronized to the .env file.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">Choose a provider preset to create a configuration draft.</p>
              </div>
              <Badge variant="default" className="border-[var(--settings-border)] bg-[var(--settings-surface-hover)] text-muted-text">{channels.length} channel{channels.length === 1 ? '' : 's'}</Badge>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-text">Choose a provider preset, then select Add channel to begin.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-text">Primary, fallback, vision, and temperature settings are written directly to the runtime configuration.</p>
                </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  Add at least one enabled channel with a model before configuring primary, fallback, and vision models.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.primaryModel, 'Automatic (use the first available model)')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Automatic (inherit the standard analysis model)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-secondary-text">
                      Fallback models are used only when the primary model fails. The primary model cannot also be a fallback.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Automatic (use the default vision selection)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
            ...
            <InlineAlert
              variant="warning"
              message="An advanced model-routing YAML configuration is active. This section manages only channel entries and basic connection details. Runtime primary, fallback, vision, and temperature settings still come from the general fields below. When the YAML parses successfully, its route and model declarations take precedence; these settings do not overwrite the YAML file."
