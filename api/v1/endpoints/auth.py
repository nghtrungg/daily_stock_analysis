...
async def auth_update_settings(request: Request, body: AuthSettingsRequest):
    ...
    if target_enabled:
        if password or confirm:
            if stored_password_exists:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": "An administrator password already exists. Enable authentication and use Change password to update it.",
                    },
                )
            if not password:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_required", "message": "Enter the administrator password to set"},
                )
            if password != confirm:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_mismatch", "message": "The password confirmation does not match"},
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "password_required", "message": "Set a password before enabling password login"},
            )
        else:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Enter the current password before re-enabling authentication"},
                    )
                ...
                if not verify_stored_password(current_password):
                    ...
                    return JSONResponse(
                        status_code=401,
                        content={"error": "invalid_password", "message": "The current password is incorrect"},
                    )
                clear_rate_limit(ip)
    else:
        if current_enabled:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Enter the current password before disabling authentication"},
                    )
                ip = get_client_ip(request)
                if not check_rate_limit(ip):
                    return JSONResponse(
...
async def auth_login(request: Request, body: LoginRequest):
    ...
    if not password:
        return JSONResponse(
            status_code=400,
            content={"error": "password_required", "message": "Enter your password"},
        )
...
async def auth_change_password(body: ChangePasswordRequest):
    ...
    if not current:
        return JSONResponse(
            status_code=400,
            content={"error": "current_required", "message": "Enter the current password"},
        )
    if new_pwd != new_confirm:
        return JSONResponse(
            status_code=400,
            content={"error": "password_mismatch", "message": "The new-password confirmation does not match"},
        )
