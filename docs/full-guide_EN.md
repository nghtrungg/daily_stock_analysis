...
A buy result must contain four actionable numeric levels satisfying `stop_loss < ideal_buy <= secondary_buy < take_profit`. Its reward-to-risk ratio, `(take_profit - ideal_buy) / (ideal_buy - stop_loss)`, must be at least `1.5`. The system prompt requests this geometry and, when report-integrity retries are enabled, asks the model to regenerate invalid levels before deterministic repair is used.

At the final backend boundary, after any enabled retries, the trading-plan gateway repairs safe cases from the model's `ideal_buy`: an invalid stop becomes `ideal_buy * 0.96`, a missing or low secondary entry becomes `ideal_buy`, and a missing or low target starts at `ideal_buy * 1.08` before being raised when needed to satisfy the R:R floor and remain above `secondary_buy`. Every repaired plan is validated again. If `ideal_buy` is empty or a localized placeholder such as `N/A`, `TBD`, `待补充`, or `Cần bổ sung`, the gateway does not invent an anchor; the existing guardrail changes the final action to hold/watch, lowers the score into the neutral band, and synchronizes the headline, one-line decision, position advice, and action fields.

The canonical sniper-point fields remain numeric for history, API, decision-signal, and backtest consumers. New reports add `battle_plan.trading_plan_validation` with `quality_status`, stable warning codes, the calculated ratio, and `.VN` display helpers using Vietnamese dot thousands separators, such as `20.640 VND (-4.0%)`, `24.500 VND (+14.0%)`, and `R:R = 1 : 3.49`. This metadata is additive and does not rewrite older stored reports.
