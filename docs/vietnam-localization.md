...
- The final score is explained through fixed trend, momentum, volume, market, and fundamental point budgets. Evidence confidence is calculated separately from data coverage, while downside, sideways, and upside scenario probabilities are normalized to 100% and labeled uncalibrated until a versioned historical calibration exists.
- For a valid long plan, the report combines deterministic R:R with the upside scenario estimate to show a before-cost Expected Value in R units. Invalid plans do not display a win probability or EV.
